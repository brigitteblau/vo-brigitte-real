# slam_bb.py
"""
codigo para probar cuando tengas descargado el dataset 
"""
import numpy as np
import cv2
import copy
...
from visualization import init_traj_view, traj_update_from_pose  # <-- usamos init y update
...
class Tracking:
    def __init__(
        ...
        # inicializar la vista si corresponde
        if self.visualize:
            init_traj_view("Trayectoria (SLAM)")
    ...
    def track(self, img_t, depth_t, timestamp):
        ...
        if len(self.frames) == 0:
            ...
            # dibujo primera pose
            if self.visualize:
                traj_update_from_pose(keyframe.pose)  # matriz 4x4
            return
        ...
        # === DIBUJO TRAYECTORIA ===
        if self.visualize:
            # Pasamos la POSE 4x4: el visualizador extrae [x,y,z]
        ...
        if self.save_path:
            with open("poses.txt", "a") as f:
                ...
                position = pose[0:3, 3]
                quaternion = R.from_matrix(pose[0:3, 0:3]).as_quat()
...
if __name__ == "__main__":
    ...
    for depth_timestamp, rgb_timestamp in matches.items():
        ...
        depth_image[depth_image == 0] = np.nan   # missing → NaN
    ...
    tracker = Tracking(cx, cy, fx, 1, initial_pose, visualize=True, save_path=True)
