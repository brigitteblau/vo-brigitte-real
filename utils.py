...
class Tracking:
    def __init__(
        ...
        self.vo_hibrido = HybridVO(cx, cy, fx)  # contains various helper functions
    ...
    def track(self, img_t, depth_t, timestamp):
        ...
        img_gray_t = self.vo_hibrido._convert_grayscale(img_t)
        kpts_t, desc_t = self.vo_hibrido._compute_orb(img_gray_t)
        kpts_t = np.array([kpt.pt for kpt in kpts_t])  # (N, 2)
        points_3d_k = self.vo_hibrido._project_2d_kpts_to_3d(depth_t, kpts_t)  # (N, 3)
        ...
        T = self.vo_hibrido._minimize_reprojection_error(p_2d, p_3d_k)
    ...
    def get_matched_points(self, mask, prev_frame, curr_frame):
        ...
        good_matches = self.vo_hibrido._get_matches(
...
def project_points(points, pose, K):
    """
    points: (N,3) en mundo
    pose:   w_T_k (pose de cámara en mundo)
    K:      3x3 intrínseca
    """
    Pw = np.hstack([points, np.ones((points.shape[0], 1))])   # (N,4)
    k_T_w = np.linalg.inv(pose)
    Pc_h = (k_T_w @ Pw.T).T                                   # (N,4)
    Pc = Pc_h[:, :3]
    z = np.clip(Pc[:, 2], 1e-8, None)                         # evita /0
    x = Pc[:, 0] / z
    y = Pc[:, 1] / z
    u = K[0, 0] * x + K[0, 2]
    v = K[1, 1] * y + K[1, 2]
